...
if [ -n "${COMMAND}" ]; then
  ...
  # FIX: REMOVED --key="${CRYPTO_KEY}"
  echo "Configuring database access as ${CMD_MAGENTO} setup:config:set --db-host="${MYSQL_HOSTNAME}" \
              --db-name="${MYSQL_DATABASE}" --db-user="${MYSQL_USERNAME}" \
              --db-password="${MYSQL_PASSWORD}" --no-interaction"
  ...
  echo "Before installing magento"
  ...
  CMD_INSTALL="${CMD_MAGENTO} setup:install --base-url="${URI}" \
                ...
                --opensearch-host="${SEARCH_ENGINE_HOST}" \
                --opensearch-port="${SEARCH_ENGINE_PORT}" \
                --use-rewrites=1"

  # Run configuration command
  $CMD_CONFIG
  echo "After installing magento"
